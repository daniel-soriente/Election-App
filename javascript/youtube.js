/**
 * 
 * Javascript for controlling youtube view
 * Taken from http://www.labnol.org/internet/light-youtube-embeds/27941/
 * Used to speed load times
 * Instead of loading every videos youtube player
 * It will just overlay an image
 */
function youtube() {
var videos = document.getElementsByClassName("youtube");

if(videos.length > 0) {
	var s=document.createElement("style");
	s.type="text/css";
	s.innerHTML='.youtube{background-color:#000;max-width:100%;overflow:hidden;position:absolute;cursor:hand;cursor:pointer}.youtube .thumb{bottom:0;display:block;left:0;margin:auto;max-width:100%;position:absolute;right:0;top:0;width:100%;height:auto}.youtube .play{filter:alpha(opacity=80);opacity:.8;height:77px;left:50%;margin-left:-38px;margin-top:-38px;position:absolute;top:50%;width:77px;background:url("data:image/png;base64,") no-repeat}';

	document.body.appendChild(s)
}

for(var n=0; n < videos.length; n++){
	var y=videos[n];
	var i=document.createElement("img");
	i.setAttribute("src","http://i.ytimg.com/vi/"+y.id+"/hqdefault.jpg");
	i.setAttribute("class","thumb");
	var c=document.createElement("div");
	c.setAttribute("class","play");
	y.appendChild(i);
	y.appendChild(c);
	y.onclick=function() {
		var yt_back=document.createElement("DIV");
		yt_back.id="yt_back";
		yt_back.style.backgroundColor="#000000";
		yt_back.onclick = function () {
    			this.parentElement.removeChild(document.getElementById("yt"));
    			this.parentElement.removeChild(this);
		};
		yt_back.style.opacity="0.9";
		yt_back.style.width="940px";
		yt_back.style.height="620px";
		yt_back.style.top="0px";
		yt_back.style.left="0px";
		document.getElementById("container").appendChild(yt_back);
		
		var yt=document.createElement("iframe");
		yt.setAttribute("src","https://www.youtube.com/embed/"+this.id+"?autoplay=1&autohide=1&border=0&wmode=opaque&enablejsapi=1");
		yt.id="yt";
		yt.style.width="700px";
		yt.style.height="500px";
		yt.style.position=this.style.position;
		yt.style.left="110px";
		yt.style.top="60px";
		document.getElementById("container").appendChild(yt);
	}
};
}
